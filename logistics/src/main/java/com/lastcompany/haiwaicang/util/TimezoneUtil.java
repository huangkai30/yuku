...
//        String sourceTime = "2017-01-06 18:30:00";
        //原时区东八区Asia/Shanghai,GMT+8:00
        String sourceId = "America/Los_Angeles";
        //目标时区Asia/Dili,GMT+09:00
        String targetId = "Asia/Shanghai";
        //转换后的时间
        Date targetTime = timeConvert(sourceTime, sourceId, targetId);
     ...
     * @param sourceDate 时间格式必须为：yyyy-MM-dd HH:mm:ss
     * @param sourceId 入参的时间的时区id
     * @param targetId 要转换成目标时区id（一般是是零时区：取值UTC）
     * @return Date 转化时区后的时间
     */
    public static Date timeConvert(Date sourceDate, String sourceId,
                                     String targetId)
    {
        //校验入参是否合法
        if (null == sourceId || "".equals(sourceId) || null == targetId
                || "".equals(targetId) || null == sourceDate
               )
        {
            return null;
        }
...
//        }
        ...
        try
        {
            String sourceTime = DateTypeHelper.getDateString(sourceDate,DateTypeHelper.DEFAULT_TIMESTAMP_PATTERN);
            //时间格式
            ...
            sourceDate = df.parse(sourceTime);
            ...
            Date targetDate =  DateTypeHelper.getDate(targetTime,DateTypeHelper.DEFAULT_TIMESTAMP_PATTERN);
            return targetDate;
        }
